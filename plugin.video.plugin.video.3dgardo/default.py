...
mysettings = xbmcaddon.Addon(id = 'plugin.video.plugin.video.3dgardo')
...
def main():
	...
	if len(online_m3u) > 0:	
		add_dir('[COLOR yellow][B]>> PELICULAS INFANTILES OLD <<[/B][/COLOR]', u_tube, 2, icon, fanart)
	if len(local_m3u) > 0:	
		add_dir('[COLOR magenta][B]>> LOCAL M3U <<[/B][/COLOR]', u_tube, 3, icon, fanart)
	if len(online_xml) > 0:	
		add_dir('[COLOR cyan][B]>> IPTV LATINO <<[/B][/COLOR]', u_tube, 4, icon, fanart)
	if len(local_xml) > 0:	
